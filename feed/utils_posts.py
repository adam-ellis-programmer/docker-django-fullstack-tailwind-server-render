...
def shuffle_posts_by_relevance(posts_queryset):
    ...
    for post in posts_list:
        score = getattr(post, 'relevance_score', 0) or getattr(post, 'interest_score', 0)
...
def get_smart_posts_queryset(user=None, fallback_limit=0.3, randomize=True):
    ...
    if targeted_count > 0:
        ...
        fallback_posts = base_posts.exclude(id__in=excluded_ids)[:fallback_count * 3]  # Get more to randomize
        ...
        if fallback_posts.exists():
            ...
            combined_ids = list(targeted_posts.values_list('id', flat=True)) + fallback_ids
            ...
            return base_posts.filter(id__in=combined_ids).annotate(
                relevance_score=Case(
                    *[When(id__in=targeted_posts.values_list('id', flat=True), then=Value(1.0))],
                    default=Value(0.5),
                    output_field=FloatField()
                )
            ).order_by('-relevance_score', '?')  # Random order within each relevance group
...
def get_posts_matching_interests(interests_list):
    ...
    ).order_by('-interest_score', '?').distinct()  # Random order within each score group
...
def mix_smart_posts_with_ads(posts_queryset, user=None, posts_per_page=10, ads_frequency=10):
    ...
    logger.info(f"Input: {len(posts_list)} smart posts, ads_frequency: {ads_frequency}")
    ...
    for i, post in enumerate(posts_list):
        ...
        if should_add_ad:
            logger.info(f"Attempting to get ad for position {len(mixed_items)}")
            ...
            if ad:
                ...
                ads_inserted += 1
                logger.info(f"✓ Inserted ad {ad.id} at position {len(mixed_items)}")
                
                # Track impression
                track_ad_impression(ad)
            else:
                logger.error("❌ Failed to get ad - get_targeted_ad returned None")
    
    logger.info(f"=== FINAL RESULT ===")
    logger.info(f"Total items: {len(mixed_items)}, Smart posts: {len(posts_list)}, Ads inserted: {ads_inserted}")
